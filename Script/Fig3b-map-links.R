...
impact.country <- bind_rows(foreach(i = 1:length(unique(indexes$country))) %do% {
  ...
  #             group_by(country) %>% 
  #             summarise(weightedegree_sum = sum(weightedegree)), 
  #           by = 'country') %>% 
  # mutate(ci95 = paste0('[', ci_low, ',', ci_high, ']'), 
  #        dependant = paste0(dependant, significance)) %>%
  # arrange(country) %>% 
  # select(country, dependant, weightedegree_sum, ci95)
...
ggplot() + 
  ...
  scale_fill_gradientn("Coefficient",
                       ...
                       labels = sprintf('%.02f', c(min(impact.country$dependant), -0.4, 0, 
                                                   0.4, max(impact.country$dependant))),
...
ggplot2::ggsave("fig/Fig3b-map-links.pdf", width = 7.5, height = 3.5)
