...
ggplot(reg_dta, aes(x = time, y = coef)) + 
  geom_rect(aes(xmin = 0.5, xmax = 4.5, ymin = -Inf, ymax = 201), fill = 'grey97') + 
  geom_hline(yintercept = 0, color = "grey50", lwd = 0.3, linetype = "dashed") +
  geom_linerange(aes(x = 0.5, ymin = -Inf, ymax = 201), color = "grey50", lwd = 0.14) +
  ...
  scale_y_continuous('Effect size on number of collaborations', expand = c(0.01, 0.01)) + 
  theme_classic() + 
...
ggsave('fig/Fig2b-event-covid.pdf', width = 6, height = 4)
